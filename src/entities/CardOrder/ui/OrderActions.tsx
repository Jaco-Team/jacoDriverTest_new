...
export function OrderActions(props: OrderActionsProps) {
  ...
  if( item.is_get == 0 ){
    return (
      <>
        <Button className="mt-5 h-auto bg-gray-400 rounded-xl" onPress={ () => dialCall(item.number) }>
          <ButtonText style={textStyle} className="p-3">{item.number}</ButtonText>
        </Button>

        <Button className="mt-5 h-auto bg-green-600 rounded-xl" onPress={ () => actionButtonOrder(1, item.id) }>
      ...
      <View>
        <HStack className="justify-between">
          {item.status_order == 6 ? null : (
            <Button className="mt-5 h-auto bg-primary-main rounded-xl" onPress={ () => setActiveConfirm(true, item.id, 'cancel', item.is_delete == 1 ? true : false) }>
        ...
        {item.status_order == 6 ? null : item.is_my === 1 && item.online_pay === 0 /*&& driver_pay === 1*/ ? (
          <HStack>
            <Button className="mt-5 h-auto bg-green-600 rounded-xl" onPress={ () => setActiveConfirm(true, item.id, 'finish', item.is_delete == 1 ? true : false) }>
              <ButtonText style={textStyle} className="p-3">Завершить</ButtonText>
            </Button>

            <Button className="mt-5 h-auto rounded-xl">
        ...
        {item.status_order == 6 ? null : (
          <Button className="mt-5 h-auto bg-yellow-400 rounded-xl" onPress={ () => setActiveConfirm(true, item.id, 'fake', item.is_delete == 1 ? true : false) }>
  ...
  return (
    <VStack>
      <Button className="mt-5 h-auto bg-gray-400 rounded-xl">
